				  ...
				  u_int8_t have_wtc, int is_event)
{
  if (!have_wtc) {
    ...
    if (what_to_count_2 & COUNT_TUNNEL_OPT_CLASS) printf("TUNNEL_OPT_CLASS  ");
    if (what_to_count_2 & COUNT_TUNNEL_OPT_TYPE)  printf("TUNNEL_OPT_TYPE  ");
			    ...
			    u_int8_t have_wtc, char *sep, int is_event)
{
  ...
  if (!have_wtc) {
    ...
    if (what_to_count_2 & COUNT_TUNNEL_OPT_CLASS) printf("%sCOUNT_TUNNEL_OPT_CLASS", write_sep(sep, &count));
    if (what_to_count_2 & COUNT_TUNNEL_OPT_TYPE)  printf("%sCOUNT_TUNNEL_OPT_TYPE", write_sep(sep, &count));
...
int main(int argc,char **argv)
{
  ...
  if (want_counter || want_match) {
	...
	else if (!strcmp(count_token[match_string_index], "tunnel_opt_class")) {
	  tmpnum = atoi(match_string_token);
	  request.ptun.tunnel_opt_class = tmpnum;
	}
	else if (!strcmp(count_token[match_string_index], "tunnel_opt_type")) {
	  tmpnum = atoi(match_string_token);
	  request.ptun.tunnel_opt_type = tmpnum;
	}
  ...
  if (want_stats || want_match) {
	...
	if (!have_wtc || (what_to_count_2 & COUNT_TUNNEL_OPT_CLASS)) {
	  if (want_output & PRINT_OUTPUT_FORMATTED) printf("%-8x          ", ptun->tunnel_opt_class);
	  else if (want_output & PRINT_OUTPUT_CSV) printf("%s%x", write_sep(sep_ptr, &count), ptun->tunnel_opt_class);
	}

	if (!have_wtc || (what_to_count_2 & COUNT_TUNNEL_OPT_TYPE)) {
	  if (want_output & PRINT_OUTPUT_FORMATTED) printf("%-8x         ", ptun->tunnel_opt_type);
	  else if (want_output & PRINT_OUTPUT_CSV) printf("%s%x", write_sep(sep_ptr, &count), ptun->tunnel_opt_type);
	}
		  ...
		  int tstamp_since_epoch, int tstamp_utc)
{
  ...
  if (wtc_2 & COUNT_GENEVE) json_object_set_new_nocheck(obj, "geneve", json_integer((json_int_t)ptun->tunnel_id));
  if (wtc_2 & COUNT_TUNNEL_OPT_CLASS) {
          sprintf(misc_str, "%x", ptun->tunnel_opt_class);
          json_object_set_new_nocheck(obj, "tunnel_opt_class", json_string(misc_str));
  }
  if (wtc_2 & COUNT_TUNNEL_OPT_TYPE) {
          sprintf(misc_str, "%x", ptun->tunnel_opt_type);
          json_object_set_new_nocheck(obj, "tunnel_opt_type", json_string(misc_str));
  }
